...
# Add startup event to load guardrails from MongoDB
@app.on_event("startup")
async def startup_event():
    """Load guardrails from MongoDB when the application starts"""
    logger.info("Application starting up - loading guardrails from MongoDB...")
    load_guardrails_from_mongodb()
...
def extract_phone_numbers_from_text(text: str) -> List[str]:
    ...
    Returns a list of normalized phone numbers (10 digits).
    Handles various formats: 9876543210, 98765-43210, +91 9876543210, etc.
    """
    if not text:
        return []
    ...
    phone_pattern = re.compile(
        r'(?:\+?91[-.\s]?)?([6-9]\d{9})\b|'  # 10-digit Indian mobile (starts with 6-9)
        r'\b(\d{3}[-.\s]?\d{3}[-.\s]?\d{4})\b|'  # US-style formatting
        r'\b([6-9]\d{2}[-.\s]?\d{3}[-.\s]?\d{4})\b|'  # Formatted Indian number
        r'\b([6-9]\d{9})\b'  # Plain 10 digits starting with 6-9
    )
    ...
    for match in matches:
        # Extract the phone number (match is a tuple, get the first non-empty group)
        ...
        if phone:
            ...
            # If less than 10, skip (invalid)
            elif len(phone_clean) < 10:
                continue
            
            # Only add if it's exactly 10 digits and starts with 6-9 (Indian mobile)
...
def get_guardrail_phone_numbers() -> List[str]:
    """
    Extract all phone numbers from guardrails (from both questions and answers).
    Returns a list of normalized 10-digit phone numbers.
    """
    with guardrails_lock:
        phone_numbers = set()
        
        for guardrail in guardrails_storage:
            question = guardrail.get("question", "").strip()
            answer = guardrail.get("answer", "").strip()
            
            # Extract phone numbers from both question and answer
            if question:
...
async def fetch_detailed_information(params: FunctionCallParams):
    ...
    try:
        ...
        # CRITICAL VALIDATION: This tool should NEVER be called without contact information
        # This is an absolute blocker - reject immediately if no contact info provided
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            response = await client.post(
                "https://cibfpreprocessor-739298578243.us-central1.run.app/query",
...
class PhoneNumberDetectorProcessor(FrameProcessor):
    """Processor that detects phone numbers in assistant text and sends them to frontend via Daily app messages.
    Only displays phone numbers that are present in guardrails."""
    
    def __init__(self, room_url: str):
        super().__init__()
        self.room_url = room_url
        self.room_name = room_url.split('/')[-1] if room_url else None
        # Pattern to match phone numbers in various formats
        ...
        self.last_detected_phone = None  # Track to avoid duplicates
        self.guardrail_phones = set()  # Set of normalized phone numbers from guardrails
        self._update_guardrail_phones()
    
    def _update_guardrail_phones(self):
        """Update the set of phone numbers from guardrails"""
        guardrail_phones = get_guardrail_phone_numbers()
        self.guardrail_phones = set(guardrail_phones)
        logger.info(f"Loaded {len(self.guardrail_phones)} phone number(s) from guardrails: {list(self.guardrail_phones)}")
    
    def _normalize_phone(self, phone: str) -> Optional[str]:
        """
        Normalize phone number to 10-digit format.
        Returns None if invalid, otherwise returns normalized 10-digit string.
        """
        if not phone:
            return None
        ...
        # Only return if it's exactly 10 digits and starts with 6-9 (Indian mobile)
        if len(phone_clean) == 10 and phone_clean[0] in '6789':
            return phone_clean
    ...
    async def process_frame(self, frame, direction):
        # CRITICAL: Only process DOWNSTREAM frames (from LLM to transport)
        # UPSTREAM frames (from transport to LLM) should pass through immediately
        # This avoids the StartFrame initialization issue with bidirectional pipelines
        
        if direction == FrameDirection.UPSTREAM:
            # Pass through all upstream frames immediately (user audio input, etc.)
            await self.push_frame(frame, direction)
            return
        
        # Now we're only dealing with DOWNSTREAM frames
        # Handle StartFrame first to initialize the processor
        if isinstance(frame, StartFrame):
            await super().process_frame(frame, direction)
            return
        
        # Pass through all other non-TextFrame frames immediately
        # This includes EndFrame, audio frames, etc.
        if not isinstance(frame, TextFrame):
            await self.push_frame(frame, direction)
            return
        
        # Refresh guardrail phones periodically (in case guardrails were updated)
        # Check every 10 frames to avoid overhead
        ...
        # Only process assistant text frames going downstream
        if direction == FrameDirection.DOWNSTREAM:
            ...
            for match in matches:
                ...
                if phone:
                    ...
                    if phone_normalized:
                        # Check if this phone number is in guardrails
                        if phone_normalized in self.guardrail_phones:
                            # Only process if different from last one
                            if phone_normalized != self.last_detected_phone:
                                self.last_detected_phone = phone_normalized
                                
                                # Format for display (XXX-XXX-XXXX)
                                phone_formatted = f"{phone_normalized[:3]}-{phone_normalized[3:6]}-{phone_normalized[6:]}"
                                
                                # Send app message to frontend via Daily REST API
                                if self.room_name:
                                    try:
                                        async with aiohttp.ClientSession() as session:
                                            async with session.post(
                                                f"https://api.daily.co/v1/rooms/{self.room_name}/send-app-message",
                                                headers={
                                                ...
                                                json={
                                                    ...
                                                    "participant_ids": ["*"]  # Send to all participants
                                                }
                                            ) as response:
                                                if response.status == 200:
                                                    logger.info(f"Sent guardrail phone number to frontend: {phone_formatted}")
                                                else:
                                                    error_text = await response.text()
                                                    logger.warning(f"Failed to send phone number: {response.status} - {error_text}")
                                    except Exception as e:
                                        logger.error(f"Error sending phone number via REST API: {e}")
                        else:
                            # Phone number detected but not in guardrails - log for debugging
                            logger.debug(f"Phone number {phone_normalized} detected but not in guardrails, skipping display")
        
        # Pass the frame through (always pass TextFrame through too)
        await self.push_frame(frame, direction)
...
def load_guardrails_from_mongodb():
    """Load guardrails from MongoDB into memory storage"""
    if not MONGODB_URI:
        logger.warning("MongoDB URI not configured, guardrails will not persist")
        return
    
    try:
        client = get_mongodb_client()
        if not client:
            logger.warning("Could not connect to MongoDB, guardrails will not persist")
            return
        
        db = client["CIBF"]
        guardrails_collection = db["guardrails"]
        
        # Load all guardrails from MongoDB
        stored_guardrails = list(guardrails_collection.find({}, {"_id": 0, "question": 1, "answer": 1}))
        
        with guardrails_lock:
            guardrails_storage.clear()
            guardrails_storage.extend(stored_guardrails)
        
        logger.info(f"Loaded {len(stored_guardrails)} guardrail(s) from MongoDB")
        client.close()
        
    except Exception as e:
        logger.error(f"Error loading guardrails from MongoDB: {e}", exc_info=True)


def save_guardrails_to_mongodb():
    """Save current guardrails from memory to MongoDB"""
    if not MONGODB_URI:
        logger.warning("MongoDB URI not configured, guardrails will not persist")
        return False
    ...
    try:
        ...
        with guardrails_lock:
            # Clear existing guardrails in MongoDB and insert current ones
            guardrails_collection.delete_many({})
            
            if guardrails_storage:
                # Insert all current guardrails
                guardrails_collection.insert_many(guardrails_storage)
                logger.info(f"Saved {len(guardrails_storage)} guardrail(s) to MongoDB")
            else:
                logger.info("No guardrails to save (storage is empty)")
        
        client.close()
        return True
        
    except Exception as e:
        logger.error(f"Error saving guardrails to MongoDB: {e}", exc_info=True)
        return False
...
"""
        ...
        detailed_info_function = FunctionSchema(
            ...
            description="MANDATORY TOOL - YOU MUST ALWAYS CALL THIS TOOL when a user asks for ANY information to be sent to their WhatsApp or email. THIS IS NOT OPTIONAL. If a user says 'send me', 'can you send', 'I want it on WhatsApp/email', or asks for any information to be delivered via WhatsApp or email, you MUST call this tool immediately. CRITICAL RESTRICTION: NEVER call this tool UNLESS you have already collected the user's phone number OR email address. This tool is COMPLETELY BLOCKED until contact information is collected. If the user requests information but hasn't provided contact info, you MUST first ask for their phone number or email address. DO NOT call this tool without contact information - it will fail. This tool is used to Send course-specific brochures and detailed information to the student via WhatsApp and/or email. IMPORTANT: This tool processes requests in the background. The tool will return a status indicating success or error. Only report errors to the student if the tool explicitly returns an error status. The tool automatically sends WhatsApp message (if phone number provided) and/or email (if email provided) to the student. At least one of phone_number or email must be provided. The tool response will indicate what was successfully sent (WhatsApp, email, or both). REMEMBER: If user asks for information to be sent, you MUST call this tool - do not skip it, do not just describe verbally.",
            properties={
                "query": {
                    "type": "string",
        ...
        context = LLMContext(
            [
                {
                    "role": "assistant",
                    "content": "Greet the caller warmly and introduce yourself as Natalie, an assistant for Chennai International Book Fair 2026. Ask 'What can I help you with? Are you a publisher?' to understand their role and needs. Then collect contact information in this EXACT order: FIRST ask for their email address (mandatory) and confirm it by spelling it out letter by letter including @ and dots. THEN ask for their mobile number (10 digits) OR confirm if they want to provide it - if provided, confirm by reciting the 10 digits back."
                }
            ]
        )
        ...
        # Store the last message index to avoid re-processing
        last_processed_message_index = [0]  # Use list to make it mutable in nested function
        guardrail_phones_set = set(get_guardrail_phone_numbers())
        detected_phones_set = set()  # Track which phones have been sent
        checker_running = [True]  # Flag to stop the checker when call ends
        
        logger.info(f"Phone number checker initialized with {len(guardrail_phones_set)} guardrail phone(s): {list(guardrail_phones_set)}")
        
        async def check_for_phone_numbers():
            """Periodically check conversation for phone numbers from guardrails"""
            logger.info("Phone number checker task started")
            check_count = 0
            while checker_running[0]:
                try:
                    ...
                    # Log every 10 checks
                    if check_count % 10 == 0:
                        logger.debug(f"Phone checker: {len(context_messages)} messages, last processed: {last_processed_message_index[0]}")
                    
                    # Process only new assistant messages
                    for idx in range(last_processed_message_index[0], len(context_messages)):
                        msg = context_messages[idx]
                        role = msg.get("role")
                        content = msg.get("content", "")
                        
                        if role == "assistant" and content:
                            logger.info(f"Checking assistant message #{idx}: '{content[:100]}...'")
                            # Extract phone numbers from the message
                            detected_numbers = extract_phone_numbers_from_text(content)
                            logger.info(f"Detected {len(detected_numbers)} phone number(s) in message: {detected_numbers}")
                            
                            # Check if any detected number is in guardrails
                            for phone in detected_numbers:
                                logger.info(f"Checking if phone {phone} is in guardrails: {phone in guardrail_phones_set}")
                                if phone in guardrail_phones_set and phone not in detected_phones_set:
                                    detected_phones_set.add(phone)
                                    logger.info(f"Found guardrail phone number in conversation: {phone}")
                                        ...
                                        async with aiohttp.ClientSession() as session:
                                            async with session.post(
                                                ...
                                                json={
                                                    ...
                                                    "recipient": "*"  # Send to all participants
                                                }
                                            ) as response:
                                                if response.status == 200:
                                                    logger.info(f"✅ Sent guardrail phone number to frontend: {phone_formatted}")
                                                    ...
                                                    error_text = await response.text()
                                                    logger.error(f"Failed to send phone: {response.status} - {error_text}")
                                    except Exception as e:
                                        logger.error(f"Error sending phone number to frontend: {e}", exc_info=True)
                    
                    last_processed_message_index[0] = len(context_messages)
                    
                except Exception as e:
                    logger.error(f"Error in phone number checker: {e}", exc_info=True)
                    await asyncio.sleep(1)
            
            logger.info("Phone number checker task stopped")
        
        # Start the phone number checker task
        phone_checker_task = asyncio.create_task(check_for_phone_numbers())
        logger.info("Phone number checker task created")
        
        @transport.event_handler("on_participant_left")
        async def on_participant_left(transport, participant, reason):
            logger.info(f"Participant left: {participant}, reason: {reason}")
            
            # Stop the phone number checker
            checker_running[0] = False
            ...
            try:
                ...
                if conversation_text.strip():
                    ...
                    try:
                        async with httpx.AsyncClient(timeout=30.0) as client:
                            response = await client.post(
                                "https://cibfpostprocessor-739298578243.us-central1.run.app/process",
...
if __name__ == "__main__":
    ...
    # Load guardrails from MongoDB on startup
    logger.info("Loading guardrails from MongoDB...")
    load_guardrails_from_mongodb()
